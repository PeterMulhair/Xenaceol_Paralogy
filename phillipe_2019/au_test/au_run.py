...
for tree in glob.glob('*contree'):
    OG = tree.split('.')[0]
    OG_log = OG.replace('_gb', '-gb')

    T1 = OG + '_T1.nwk'
    T2 = OG + '_T2.nwk'
    T3 = OG + '_T3.nwk'
    unix('cat ' + T1 + ' ' + T2 + ' ' + T3 + ' >> ' + OG + '.treels', shell=True)
    unix('cp ../../OG_data/OG_aln/' + args.cc + '/' + OG_log + '.fasta .', shell=True)

    with open(args.path + OG_log + '.fasta.log') as f:
        lines = f.read()
        info = lines.split('\n')
        for line in info:
    ...
    unix('iqtree -s ' + OG_log + '.fasta -m ' + model + ' -z ' + OG + '.treels -te ' + tree + ' -zb 10000 -zw -au', shell=True)
    unix('rm ' + OG_log + '.fasta', shell=True)
