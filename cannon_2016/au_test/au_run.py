...
#Create dir for OGs that pass or fail CC test
os.mkdir(args.cc)
os.chdir(args.cc)

#Copy gene tree files for OGs 
unix('cp ../../OG_data/OG_trees/' + args.cc + '/*contree .', shell=True)


#Run clann to trim each species hypothesis tree to match each gene tree
...
#Get each of the hypothesis trees (eg T1_tree.nwk, T2_tree.nwk, T3_tree.nwk)    
hyp_tree_list = []
...
for trees in glob.glob('*contree'):
    for hyp_tree in hyp_tree_list:
        tree_trim(trees, hyp_tree)
    

#Run AU test for each OG against each hypothesis tree        
...
#Parse AU test output and write results to file
OG_count = 0
single_support = 0
T1_sup = 0
T2_sup = 0
